...
There is no `npm install` or build step. `npm test` runs the optional Node.js calculation checks without dependencies.
...
- Export a design to JSON for a portable backup and import it later; download CSV results or use the browser's Print / save PDF option for the screen report. Auto-save uses local browser storage only; clearing browser data removes it.
...
| `app.mjs` | Browser interaction, autosave, exports and chart |
